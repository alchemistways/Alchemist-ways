...
function rewriteEntryHashes(html, dir) {
  ...
  for (const [prefix, current] of map) {
    ...
    next = next.replace(re, (full, stalePath) => {
      ...
      if (staleName === current) return full.startsWith("./") || full.startsWith("/") ? `./assets/${current}` : full;
      rewrites += 1;
...
function relativizeAssetJs(dir) {
  if (!existsSync(dir)) return;
  for (const name of readdirSync(dir)) {
    if (!/\.(js|css)$/.test(name)) continue;
    const file = join(dir, name);
    const raw = readFileSync(file, "utf8");
    const next = raw.replace(/(?<!\.)\/assets\//g, "./");
...
// TanStack SPA shells occasionally embed NULs; strip so GitHub/browsers treat the file as text.
writeFileSync(join(root, "index.html"), toRelativeAssetUrls(html.replace(/\u0000/g, "")));
relativizeAssetJs(assetsDir);

for (const file of ["favicon.png", "robots.txt", "CNAME"]) {
...
console.log(
  `Synced Pages root from ${label}${rewrites ? ` (rewrote ${rewrites} stale entry hash(es))` : ""}`,
);
